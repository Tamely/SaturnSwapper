...
internal sealed class TypeSymbolBinder : TypeBinder
{
    ...
    internal TypeSymbolBinder(Binder binder, TypeSymbol type) 
        : base(binder)
    {
        _type = type;
    ...
    private MemberSymbol ResolveField(FieldSymbol fieldSymbol)
    {
        ...
        return new FieldSymbol(fieldSymbol.ParentType, fieldSymbol.Name, resolvedType, fieldSymbol.Modifiers, fieldSymbol.Offset);
    }
    ...
    public void BindMembers()
    {
        ...
        if (_type is ArrayTypeSymbol arrayTypeSymbol)
        {
            foreach (var member in arrayTypeSymbol.Members)
            {
                var boundMember = BindMember(member, SyntaxNode.Empty);
                if (boundMember is not BoundTemplateMethod)
                {
