...
namespace UAssetAPI.PropertyTypes.Objects
{
    ...
    public class TextPropertyData : PropertyData<FString>
    {
        ...
        private void SerializeFString(FString String, List<byte> data)
        {
            ...
            {
                string nullTerminatedStr = String.Value + "\0";
                data.AddRange(BitConverter.GetBytes(String.Encoding is UnicodeEncoding ? -nullTerminatedStr.Length : nullTerminatedStr.Length));
                byte[] actualStrData = String.Encoding.GetBytes(nullTerminatedStr);
        ...
        public override byte[] Serialize(UnrealPackage Asset)
        {
            ...
            if (Asset.ObjectVersion < ObjectVersion.VER_UE4_FTEXT_HISTORY)
            {
                SerializeFString(CultureInvariantString, data);
                if (Asset.ObjectVersion >= ObjectVersion.VER_UE4_ADDED_NAMESPACE_AND_KEY_DATA_TO_FTEXT)
                {
                    SerializeFString(Namespace, data);
                    SerializeFString(Value, data);
                }
                else
                {
                    SerializeFString(Value, data);
            ...
            data.AddRange(BitConverter.GetBytes((uint)Flags));

            if (Asset.ObjectVersion >= ObjectVersion.VER_UE4_FTEXT_HISTORY)
            {
                data.Add((byte)HistoryType);
                ...
                switch (HistoryType)
                {
                    ...
                    case TextHistoryType.Base:
                        ...
                        break;
                    case TextHistoryType.StringTableEntry:
                        data.AddRange(BitConverter.GetBytes(TableId.Index));
                        data.AddRange(BitConverter.GetBytes(TableId.Number));
                        SerializeFString(Value, data);
                        break;
                    case TextHistoryType.RawText:
                        SerializeFString(Value, data);
