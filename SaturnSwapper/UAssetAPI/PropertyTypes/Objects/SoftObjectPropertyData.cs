...
namespace UAssetAPI.PropertyTypes.Objects
{
    ...
    public struct FSoftObjectPath
    {
        ...
        public byte[] Serialize(UnrealPackage Asset)
        {
            List<byte> data = new();
            
            data.AddRange(BitConverter.GetBytes(AssetPath.PackageName.Index));
            data.AddRange(BitConverter.GetBytes(AssetPath.PackageName.Number));

            if (AssetPath.AssetName.Index > -1)
            {
                data.AddRange(BitConverter.GetBytes(AssetPath.AssetName.Index));
                data.AddRange(BitConverter.GetBytes(AssetPath.AssetName.Number));
            }

            if (string.IsNullOrWhiteSpace(SubPathString.Value))
            {
                data.AddRange(BitConverter.GetBytes(0));
            }
            else
            {
                string nullTerminatedStr = SubPathString.Value + "\0";
                data.AddRange(BitConverter.GetBytes(SubPathString.Encoding is UnicodeEncoding ? -nullTerminatedStr.Length : nullTerminatedStr.Length));
                byte[] actualStrData = SubPathString.Encoding.GetBytes(nullTerminatedStr);
