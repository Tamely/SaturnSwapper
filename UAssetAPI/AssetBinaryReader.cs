...
namespace UAssetAPI
{
    ...
    public class AssetBinaryReader : UnrealBinaryReader
    {
        ...
        public AssetBinaryReader(byte[] buffer, UnrealPackage asset = null) : base(new MemoryStream(buffer))
        {
            Buffer = buffer;
            Asset = asset;
        }
        ...
        {
            T obj = Unsafe.ReadUnaligned<T>(ref Buffer[BaseStream.Position]);
            BaseStream.Position += sizeof(T);
            return obj;
        }
        
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe T[] ReadArray<T>(int length) where T : unmanaged
        {
            if (length == 0)
            ...
            Unsafe.CopyBlockUnaligned(ref Unsafe.As<T, byte>(ref objArray[0]), ref Buffer[BaseStream.Position], (uint) byteCount);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe T[] ReadArray<T>(int length, Func<T> getter)
        {
            if (length == 0)
                return Array.Empty<T>();
