...
namespace UAssetAPI.PropertyTypes.Structs
{
    ...
    {        
        ...
        public override byte[] Serialize(UnrealPackage Asset)
        {
            ...
            if (Asset.ObjectVersionUE5 >= ObjectVersionUE5.LARGE_WORLD_COORDINATES)
            {
                data.AddRange(BitConverter.GetBytes(Value.Pitch));
                data.AddRange(BitConverter.GetBytes(Value.Yaw));
                data.AddRange(BitConverter.GetBytes(Value.Roll));
            }
            else
            {
                data.AddRange(BitConverter.GetBytes(Value.PitchFloat));
                data.AddRange(BitConverter.GetBytes(Value.YawFloat));
                data.AddRange(BitConverter.GetBytes(Value.RollFloat));
            }
