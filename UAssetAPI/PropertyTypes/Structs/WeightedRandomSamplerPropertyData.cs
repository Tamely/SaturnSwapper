...
namespace UAssetAPI.PropertyTypes.Structs
{
    public class WeightedRandomSamplerPropertyData : PropertyData
    {
        ...
        public override byte[] Serialize(UnrealPackage Asset)
        {
            ...
            data.AddRange(BitConverter.GetBytes(Prob.Length));
            foreach (var entry in Prob)
            {
                data.AddRange(BitConverter.GetBytes(entry));
            }
            
            data.AddRange(BitConverter.GetBytes(Alias.Length));
            foreach (var entry in Alias)
            {
                data.AddRange(BitConverter.GetBytes(entry));
            }
            
            data.AddRange(BitConverter.GetBytes(TotalWeight));
