...
namespace UAssetAPI.PropertyTypes.Objects
{
    ...
    public class MapPropertyData : PropertyData
    {
        ...
        public override byte[] Serialize(UnrealPackage Asset)
        {
            ...
            data.AddRange(BitConverter.GetBytes(KeysToRemove?.Length ?? 0));
            if (KeysToRemove != null)
            {
                foreach (var entry in KeysToRemove)
                {
                    data.AddRange(entry.Serialize(Asset));
                }
            }

            data.AddRange(BitConverter.GetBytes(Value.Count));
            
            foreach (var entry in Value)
            {
                data.AddRange(entry.Key.Serialize(Asset));
                data.AddRange(entry.Value.Serialize(Asset));
            }
