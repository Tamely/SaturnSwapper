...
namespace UAssetAPI.PropertyTypes.Objects
{
    ...
    public class EnumPropertyData : PropertyData<FName>
    {
        ...
        public override byte[] Serialize(UnrealPackage Asset)
        {
            ...
            if (InnerType?.Value?.Value == "ByteProperty")
            {
                int enumIndice = Value == null ? byte.MaxValue : (byte)Asset.Mappings.EnumMap[EnumType.Value.Value].Values.Where(pair => pair.Value == Value.Value.Value).Select(pair => pair.Key).FirstOrDefault(); // wow this code is stupid
                data.Add((byte)enumIndice);
            }
            else
            {
