...
namespace UAssetAPI.PropertyTypes.Objects
{
    ...
    public class SetPropertyData : ArrayPropertyData
    {
        ...
        public override byte[] Serialize(UnrealPackage Asset)
        {
            ...
            if (Value.Length > 0) ArrayType = new FName(Asset, Value[0].PropertyType);

            var removedItemsDummy = new ArrayPropertyData(FName.DefineDummy(Asset, "ElementsToRemove"))
                {
                    ShouldSerializeStructsDifferently = false,
                    ArrayType = ArrayType,
            ...
            data.AddRange(removedItemsDummy.Serialize(Asset));
            data.AddRange(base.Serialize(Asset));
