...
namespace UAssetAPI
{
    public abstract class UnrealPackage : INameMap
    {
        ...
        public bool ContainsNameReference(FString search)
        {
            ...
            {
                return nameMapLookup.ContainsKey(search.Value.Split('.')[0]) && nameMapLookup.ContainsKey(search.Value.Split('.')[1]);
            }
        ...
        {
            //FixNameMapLookupIfNeeded();
            if (ContainsNameReference(search)) return nameMapLookup[search.Value.Split('.')[0]];
