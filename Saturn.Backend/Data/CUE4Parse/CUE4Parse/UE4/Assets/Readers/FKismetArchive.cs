...
public class FKismetArchive : FArchive
{
    ...
    public override FName ReadFName()
    {
        ...
        if (nameIndex < 0 || nameIndex >= Owner.NameMap.Count)
        {
            throw new ParserException(this, $"FName could not be read, requested index {nameIndex}, name map size {Owner.NameMap.Count}");
        }
#endif
