...
Document FSkinGenerator::json;

std::vector<FItem> FSkinGenerator::GetItems() {
	std::vector<FItem> items;

	static std::tuple<int, std::string> stringData = WindowsFunctionLibrary::GetRequest("https://fortnite-api.com/v2/cosmetics/br/search/all?backendType=AthenaCharacter");
...
std::vector<FItem> FSkinGenerator::FilterItems(const std::string& filter) {
	if (filter.empty()) {
		return GetItems();
	}
...
FItem FSkinGenerator::GetItemById(const std::string& id) {
	FItem item;
