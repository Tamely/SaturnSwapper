...
{
	uint32_t Key[4 * (AES256_ROUND_COUNT + 1)];
};
...
static const uint8_t AesISbox[256] =
{
...
static const uint32_t AesITbox[4][256] =
{
	{
...
static inline uint32_t AesEncryptMix(uint32_t x)
{
	return ((uint32_t)AesSbox[(uint8_t)(x >> 0)] << 0)
		^ ((uint32_t)AesSbox[(uint8_t)(x >> 8)] << 8)
		^ ((uint32_t)AesSbox[(uint8_t)(x >> 16)] << 16)
		^ ((uint32_t)AesSbox[(uint8_t)(x >> 24)] << 24);
}

static inline uint32_t AesDecryptMix(uint32_t x)
{
	return (AesITbox[0][AesSbox[(uint8_t)(x >> 0)]])
		^ (AesITbox[1][AesSbox[(uint8_t)(x >> 8)]])
		^ (AesITbox[2][AesSbox[(uint8_t)(x >> 16)]])
		^ (AesITbox[3][AesSbox[(uint8_t)(x >> 24)]]);
}

static inline uint32_t RotateRight(uint32_t X, int N)
{
	return (X >> N) | (X << (32 - N));
}

static inline void AesEncryptExpand(FAesExpandedKey* EncryptKey, const uint8_t* Key)
{
	...
	{
		uint32_t Rcon = 1U << Index;

		EKey[8] = EKey[0] ^ RotateRight(AesEncryptMix(EKey[7]), 8) ^ Rcon;
...
static inline void AesDecryptExpand(FAesExpandedKey* DecryptKey, const uint8_t* Key)
{
	FAesExpandedKey EncryptKey;
	AesEncryptExpand(&EncryptKey, Key);

	auto EKey = EncryptKey.Key + (4 * AES256_ROUND_COUNT);
	auto DKey = DecryptKey->Key;
	...
	{
		DKey[0] = AesDecryptMix(EKey[0]);
		DKey[1] = AesDecryptMix(EKey[1]);
		DKey[2] = AesDecryptMix(EKey[2]);
		DKey[3] = AesDecryptMix(EKey[3]);
		DKey += 4;
		EKey -= 4;
	}
...
void FAESKey::DecryptData(uint8_t* Contents, uint32_t NumBytes) const
{
#define AES_DEC(a, b, c, d) (       \
	AesITbox[0][(uint8_t)(a >>  0)] ^ \
	AesITbox[1][(uint8_t)(b >>  8)] ^ \
	AesITbox[2][(uint8_t)(c >> 16)] ^ \
	AesITbox[3][(uint8_t)(d >> 24)]   \
)

#define AES_DEC_LAST(a, b, c, d) (       \
	(AesISbox[(uint8_t)(a >>  0)] <<  0) ^ \
	(AesISbox[(uint8_t)(b >>  8)] <<  8) ^ \
	(AesISbox[(uint8_t)(c >> 16)] << 16) ^ \
	(AesISbox[(uint8_t)(d >> 24)] << 24)   \
)

	FAesExpandedKey DecryptKey;
	AesDecryptExpand(&DecryptKey, this->Key);

	auto DKey = DecryptKey.Key;

	for (uint32_t Offset = 0; Offset < NumBytes; Offset += 16)
	{
		...
		for (int Round = 1; Round < AES256_ROUND_COUNT; Round++)
		{
			uint32_t t0 = AES_DEC(s0, s3, s2, s1);
			uint32_t t1 = AES_DEC(s1, s0, s3, s2);
			uint32_t t2 = AES_DEC(s2, s1, s0, s3);
			uint32_t t3 = AES_DEC(s3, s2, s1, s0);
		...
		uint32_t t0 = AES_DEC_LAST(s0, s3, s2, s1);
		uint32_t t1 = AES_DEC_LAST(s1, s0, s3, s2);
		uint32_t t2 = AES_DEC_LAST(s2, s1, s0, s3);
		uint32_t t3 = AES_DEC_LAST(s3, s2, s1, s0);

		s0 = t0 ^ DKey[4 * AES256_ROUND_COUNT + 0];
		s1 = t1 ^ DKey[4 * AES256_ROUND_COUNT + 1];
